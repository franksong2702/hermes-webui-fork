...
def test_pet_slash_command_is_registered_with_wakeup_sleep_subcommands():
    ...
    assert "subArgDescs:{wakeup:t('cmd_pet_wakeup_desc'),sleep:t('cmd_pet_sleep_desc')}" in COMMANDS_JS
    assert "desc:parsed.command.subArgDescs[String(opt)]||parsed.command.desc" in COMMANDS_JS
...
def test_pet_slash_command_reuses_settings_toggle_path():
    ...
    assert "toggleDesktopPetFromAppearance(true)" in body
    assert "toggleDesktopPetFromAppearance(false)" in body
    assert "startDesktopPet()" in body
...
def test_pet_slash_command_matches_existing_settings_checkbox_semantics():
